...
function addSinglePreviewSlotEl(btn){
  ...
  var s = k && window._ps && window._ps[k];
  if(s) addSinglePreviewSlot(s.name, s.shift, s.cap, s.hat);
}
...
function initApp(){
  ...
  if(el) el.textContent = 'Enter your 7-digit member ID';
        '<div class="preview-status">' +
          (isAdded ? '<span style="color:#155724;font-size:11px;font-weight:600">&#10003; Added</span>'
            : (function(){ var k=encodeURIComponent(s.name)+'|'+sh; window._ps=window._ps||{}; window._ps[k]={name:s.name,shift:sh,cap:s.cap,hat:s.hat}; return '<button class="btn btn-sm" style="font-size:11px;padding:3px 10px" data-k="'+k+'" onclick="addSinglePreviewSlotEl(this)">&#43; Add</button>'; })()
          ) +
        '</div>' +
      ' System — Main Application Script
// Loaded via <script src="/js/app.js"></script>
